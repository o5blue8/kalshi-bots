...
LOG_FILE = "audit_b_breakout.log"
BASE_URL = "https://api.elections.kalshi.com/trade-api/v2"
KALSHI_WS_URL = "wss://api.elections.kalshi.com/trade-api/ws/v2"

# PARAMETERS - BOT B (HIGH CONVICTION BREAKOUT)
DRY_RUN = True
MAX_TRADES_PER_WINDOW = 1
MIN_ENTRY_PRICE_CENTS = 40
MAX_ENTRY_PRICE_CENTS = 60
PROFIT_TARGET_CENTS = 15      # Higher profit target (+15c)
STOP_LOSS_CENTS = 12          # Wider stop-loss (-12c) to withstand order book wicks
PHASE1_CUTOFF = 180           # Standard 3-min window
PHASE2_CUTOFF = 600           # Extended window (up to 10 mins) if move is massive
MAX_HOLD_SECONDS = 120        # Extended hold time
MIN_BTC_MOVE_PHASE1 = 25.0    # Demands a strong $25 BTC move in Phase 1
MIN_BTC_MOVE_PHASE2 = 45.0    # Demands a massive $45 BTC move in Phase 2
...
async def run_flipper():
    log_audit(f"ENGINE STARTED [BOT B - HIGH BREAKOUT] | P1 Move: ${MIN_BTC_MOVE_PHASE1} | P2 Move: ${MIN_BTC_MOVE_PHASE2} | Stop: -{STOP_LOSS_CENTS}c")
    while True:
        engine = KalshiEngine()
        target_market = None
            ...
            async with websockets.connect(KALSHI_WS_URL, additional_headers=ws_headers) as ws:
                ...
                async for msg in ws:
                    ...
                    if data.get("type") == "cfbenchmarks_value":
                        ...
                        if elapsed >= PHASE2_CUTOFF and not logged_end:
                            if in_pos:
                                val = ob_data['yes_bid_cents'] if pos_data["side"] == "YES" else ob_data['no_bid_cents']
                                pl = val - pos_data['entry_price']
                                total_pl += pl
                                log_audit(f"⏱️ EXTENDED CUTOFF (600s): Forced exit {pos_data['side']} @ {val}c (P/L: {pl:+d}c)")
                                in_pos = False
                            log_audit(f"🏁 ENTRY PHASE CLOSED | Trades: {trade_cnt} | Net P/L: {total_pl:+d}c\n")
                            logged_end = True

                        if elapsed >= 900: break

                        if not in_pos and elapsed < PHASE2_CUTOFF and trade_cnt < MAX_TRADES_PER_WINDOW and brti and initial_brti:
                            delta = brti - initial_brti
                            req_move = MIN_BTC_MOVE_PHASE1 if elapsed < PHASE1_CUTOFF else MIN_BTC_MOVE_PHASE2
                            if abs(delta) >= req_move:
                                ...
                                if MIN_ENTRY_PRICE_CENTS <= ask <= MAX_ENTRY_PRICE_CENTS:
                                    ...
                                    log_audit(f"🎯 ENTRY #{trade_cnt} [{'P1' if elapsed < 180 else 'P2'}]: Bought {side} @ {ask}c (Move: ${delta:+.2f} | Target: {pos_data['target_sell']}c | Stop: {pos_data['stop_price']}c)")
                        ...
                        if in_pos:
                            ...
                            if bid >= pos_data["target_sell"]:
                                ...
                                pl = bid - pos_data['entry_price']
                                total_pl += pl
                                log_audit(f"⏱️ HOLD EXPIRED (120s): Closed {side} @ {bid}c (P/L: {pl:+d}c)")
                                in_pos, initial_brti = False, brti
        except Exception as e:
            log_audit(f"Bot B Exception: {e}. Reconnecting in 3s...")
