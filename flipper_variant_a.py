...
LOG_FILE = "audit_a_baseline.log"
BASE_URL = "https://api.elections.kalshi.com/trade-api/v2"
KALSHI_WS_URL = "wss://api.elections.kalshi.com/trade-api/ws/v2"

# PARAMETERS - BOT A (BASELINE)
DRY_RUN = True
...
class KalshiEngine:
    ...
    def get_balance(self):
        res = requests.get(f"{BASE_URL}/portfolio/balance", headers=self.get_headers("GET", "/trade-api/v2/portfolio/balance"))
        return res.json().get("balance", 0) / 100.0 if res.status_code == 200 else None
...
async def run_flipper():
    log_audit(f"ENGINE STARTED [BOT A - BASELINE] | Range: {MIN_ENTRY_PRICE_CENTS}-{MAX_ENTRY_PRICE_CENTS}c | Move: ${MIN_BTC_MOVE_USD}")
    while True:
        engine = KalshiEngine()
        target_market = None
        ...
        try:
            async with websockets.connect(KALSHI_WS_URL, additional_headers=ws_headers) as ws:
                ...
                async for msg in ws:
                    ...
                    if data.get("type") == "cfbenchmarks_value":
                        ...
                        if not in_pos and elapsed < WINDOW_ACTIVE_SECONDS and trade_cnt < MAX_TRADES_PER_WINDOW and brti and initial_brti:
            ...
            log_audit(f"Bot A Exception: {e}. Reconnecting in 3s...")
